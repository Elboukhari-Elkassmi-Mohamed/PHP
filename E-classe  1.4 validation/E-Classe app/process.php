...
if (isset($_POST['sing_Up'])){
    ...
    $result = $mysqli->query("SELECT * FROM comptes WHERE Email='$email'");
    
  if ($result->num_rows<1)
{
    $mysqli->query("INSERT INTO comptes (Email ,web_pass ,ad_name) VALUES ('$email','$password','$name')")
     or die($mysqli->error);
    ...
    if (isset($_POST['save'])) {
        ...
        if(empty($name) && empty($email) && empty($phone) && empty($Number) && empty($Date)){
            $mysqli->close();
         header("Location: students.php");
        }else{

        $mysqli->query("INSERT INTO students (name, email ,phone , Number, Date) VALUES ('$name', '$email','$phone','$Number','$Date')")
            or die($mysqli->error);
